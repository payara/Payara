...
/**
 * Runtime representation of a server pool, with its members and such.
 */
public class LocalServerPool implements PhysicalServerPool, ConfigListener {
    ...
    public LocalServerPoolFactory getServerPoolFactory() {
        return serverPoolFactory;
    }
    
    @Override
    public Collection<VirtualMachine> getVMs() throws VirtException {
        List<VirtualMachine> vms1 = new ArrayList<VirtualMachine>();
        for (Machine machine : machines.values()) {
            vms1.addAll(machine.getVMs());
        }
        return vms1;
    }
    ...
    private void populateGroup() {
        ...
        for (MachineConfig machineConfig : config.getMachines()) {
            if (cached.containsKey(machineConfig.getName())) {
                ...
                Machine machine = (machineConfig.getNetworkName()!=null && NetUtils.isThisHostLocal(machineConfig.getNetworkName())?
                        LocalMachine.from(injector, this, machineConfig):
                        RemoteMachine.from(injector, this, machineConfig, ipAddress));
                if (machine.isUp()) {
                    addMachine(machine);
                    continue;
                }
            }
            // the cache did not contain an entry for this machine or the cached IP address is not responding, lets be safe.
