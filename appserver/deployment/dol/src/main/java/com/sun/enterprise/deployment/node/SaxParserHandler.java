...
public class SaxParserHandler extends DefaultHandler {
    ...
    protected static List<VersionUpgrade> getVersionUpgrades(String key) {
      ...
      versionUpgradeList = new ArrayList<>();
        for (Class aClass : classList) {
            ...
            try {
                versionUpgrade = (VersionUpgrade) aClass.newInstance();
            } catch (Exception ex) {
            }
    ...
    public InputSource resolveEntity(String publicID, String systemID) throws SAXException {
        try {
            ...
            if (publicID==null) {
                ...
                String fileName = namespaceResolution != null
                    ? getSchemaURLFor(namespaceResolution)
                    : getSchemaURLFor(systemID.substring(systemID.lastIndexOf('/')+1));
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) {
        if( !pushedNamespaceContext ) {
            // We need one namespace context per element, so push a context
