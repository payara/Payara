...
public class PhaseMethod {
    ...
    public PhaseMethod(Method method, PhaseType phaseType) {
        ...
        if (priority != null) {
            this.explicitlyOrdered = true;
            this.sortKey = priority.value();
        } else if (phaseType == PhaseType.DECISION) {
            int order = method.getAnnotation(Decision.class).order();
            this.explicitlyOrdered = (order != 0);
            this.sortKey = order;
        } else {
            int order = method.getAnnotation(Action.class).order();
            this.explicitlyOrdered = (order != 0);
            this.sortKey = order;
        }
    }
