...
/**
 * LargeLanguageModel implementation for Anthropic Claude API.
 * <p>
 * Supports Claude 3 models (Opus, Sonnet, Haiku) and Claude 2.
 * <p>
 * Configuration:
 * <ul>
 *     <li>jakarta.ai.llm.provider=anthropic</li>
 *     <li>jakarta.ai.llm.api-key=sk-ant-...</li>
 *     <li>jakarta.ai.llm.model=claude-3-opus-20240229 (or claude-3-sonnet, claude-3-haiku)</li>
 * </ul>
 *
 * @author Luis Neto
 */
public class AnthropicLargeLanguageModel implements LargeLanguageModel {

    private static final Logger logger = Logger.getLogger(AnthropicLargeLanguageModel.class.getName());
    ...
    public AnthropicLargeLanguageModel(LLMConfiguration config) {
        this.config = config;
    ...
    private String executeQuery(String prompt, String systemPrompt) {
        try {
            String endpoint = config.endpoint() != null ? config.endpoint() : LLMProvider.ANTHROPIC.getDefaultEndpoint();
            String model = config.model() != null ? config.model() : DEFAULT_MODEL;
            String requestBody = buildRequestBody(prompt, systemPrompt, model);

            logger.log(Level.FINE, "Sending request to Anthropic API: {0}", endpoint);
            ...
            logger.log(Level.SEVERE, "Error communicating with Anthropic API", e);
    ...
    private String buildRequestBody(String prompt, String systemPrompt, String model) {
        ...
        json.append("\"max_tokens\": ").append(DEFAULT_MAX_TOKENS).append(",");
    ...
    private String extractResponseContent(String responseBody) {
        ...
        try {
            ...
            if (contentStart == -1) {
                // Fallback: try to find content array
            ...
            contentStart = responseBody.indexOf("\"", contentStart + 7) + 1;
            ...
            logger.log(Level.WARNING, "Error parsing Anthropic response", e);
            return responseBody;
        }
    }
