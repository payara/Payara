...
public class EjbBundleNode extends AbstractBundleNode<EjbBundleDescriptorImpl> {
   ...
   public void addDescriptor(Object newDescriptor) {
       if (newDescriptor instanceof EjbDescriptor) {
           ...
           for (MethodDescriptor anArray : array) {
               EjbDescriptor target = descriptor.getEjbByName(anArray.getEjbName());
               MethodPermission[] mps = nd.getMethodPermissions();
               for (MethodPermission mp : mps) {
                   DOLUtils.getDefaultLogger().fine("Adding mp " + mp + " to " + anArray + " for ejb " + anArray.getEjbName());
                   target.addPermissionedMethod(mp, anArray);
               }
           }
    ...
    private void writeAssemblyDescriptor(Node parentNode, EjbBundleDescriptorImpl bundleDescriptor) {
       ...
       // method-permission*
       Map<EjbDescriptor, Set<MethodDescriptor>> excludedMethodsByEjb = new HashMap<>();
       ...
       for (EjbDescriptor ejbDesc : bundleDescriptor.getEjbs()) {
           if (ejbDesc instanceof EjbMessageBeanDescriptor)
               continue;
           Set<MethodDescriptor> excludedMethods = new HashSet<>();
       ...
       if (excludedMethodsByEjb.size()>0) {
           ...
           for (Map.Entry<EjbDescriptor, Set<MethodDescriptor>> entry : excludedMethodsByEjb.entrySet()) {
               EjbDescriptor ejbDesc = entry.getKey();
               Set<MethodDescriptor> excludedMethods = entry.getValue();
    ...
    private void addMethodPermissions(
            ...
            Set<MethodDescriptor>  excludedMethods,
        ...
        for (Map.Entry<MethodPermission, Set<MethodDescriptor>> entry : mpToMethods.entrySet()) {
            ...
            if (mp.isExcluded()) {
                ...
                excludedMethods.addAll(entry.getValue());
            } else {
                MethodPermissionDescriptor mpd = new MethodPermissionDescriptor();
                mpd.addMethodPermission(mp);
                mpd.addMethods(entry.getValue());
