 ...
 */
@Service
@AnnotationHandlerFor(ConnectorResourceDefinitions.class)
    ...
    protected HandlerProcessingResult processAnnotation(AnnotationInfo ainfo,  ResourceContainerContext[] rcContexts)
        ...
        if(values != null && values.length >0){
            for(ConnectorResourceDefinition defn : values){
                String defnName = ConnectorResourceDefinitionDescriptor.getJavaName(defn.name());

                if(duplicates.contains(defnName)){
                    // where is the local-string file?
                    String localString = localStrings.getLocalString(
                            "enterprise.deployment.annotation.handlers.connectorresourcedefinitionsduplicates",
