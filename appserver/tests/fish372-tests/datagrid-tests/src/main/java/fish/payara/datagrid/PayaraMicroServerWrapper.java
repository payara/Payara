 ...
 * {@link ClassLoader}.
 * <p>
 * Tests using the {@link PayaraMicroServerWrapper} should call {@link #start()} before the test(s) and {@link #stop()} after.
 */
public final class PayaraMicroServerWrapper
{
  static
  {
  ...
  private static final PayaraMicroServerWrapper
    INSTANCE = new PayaraMicroServerWrapper();

  public static PayaraMicroServerWrapper newInstance()
  {
    System.out.println (">>>>> PayaraMicroServerWrapper.newInstance()");
    return INSTANCE;
  }

  private static final String GLASSFISH_CLASS_NAME =
    "org.glassfish.embeddable.GlassFish";
  private static final String SERVICE_LOCATOR_CLASS_NAME =
  ...
  private PayaraMicroServerWrapper()
  {
  }
  ...
  public void start(String... args)
  {
    try
    {
      ...
      if (!isStarting.compareAndSet(false, true))
      {
        ...
        {
          throw new IllegalStateException("Previous start was not successful.");
        }
      }
  ...
  private void start(String[] args, PayaraMicroBoot boot, int port)
    throws Exception
  {
    this.options = args;
    this.httpPort = port;
  ...
  public <T> T getExtensionByType(String target, Class<T> type)
  {
    ...
    {
      Method getConfig =
        targetUtil.getClass().getMethod("getConfig", String.class);
      Object config = getConfig.invoke(targetUtil, target);
      Method getExtensionByType =
        config.getClass().getMethod("getExtensionByType", Class.class);
  ...
  {
    return e.getClass() == AssertionError.class ? (AssertionError) e :
