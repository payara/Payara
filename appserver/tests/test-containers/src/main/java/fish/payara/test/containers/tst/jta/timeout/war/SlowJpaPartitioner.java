...
import fish.payara.test.containers.tst.jta.timeout.war.entities.TestEntity;
...
public class SlowJpaPartitioner {
    ...
    private static final Logger LOG = Logger.getLogger(SlowJpaPartitioner.class.getName());
    ...
    public void slowlyPreparePartition(){
        LOG.info("slowlyPreparePartition()");
        try {
            persist("Entity 1");
            LOG.info("Simulating time consuming work.");
            // to be sure we exceeded allowed time
            Thread.sleep(TIMEOUT_IN_SECONDS * 1000L + 300L);
            LOG.info("Time consuming work finished.");
        } catch (InterruptedException ex) {
            // test broken, you should not get here.
            LOG.log(Level.SEVERE, "Thread Interruption", ex);
        } finally {
            em.flush();
        }
    }
    ...
    public void executePreparedPartition() {
        LOG.info("executePreparedPartition()");
        persist("Entity 2");
        em.flush();
    }
}
