...
package test.deploymentgroupsetup;

import com.sun.appserv.test.AdminBaseDevTest;
import org.testng.annotations.Test;
import org.testng.Assert;

public class DeploymentGroupSetupTest extends AdminBaseDevTest {
    ...
    public void createDeploymentGroupTest() throws Exception {
        ...
        // Try to see if there's an existing group config, if so tear it down
        try {
            deleteInstance();
        ...
        // Create a deployment group and two instances
        retStatus = report(tn + "create-deployment-group", asadmin("create-deployment-group", cname));
        Assert.assertEquals(retStatus, true, "Create Deployment Group failed ...");
    }

    @Test(dependsOnMethods = { "createDeploymentGroupTest" })
    public void createInstanceTest() throws Exception {
        report(tn + "create-local-instance1",
                asadmin(
                    "create-local-instance",
                    "--deploymentgroup", cname,
    ...
    public void deleteDeploymentGroup() throws Exception {
        AsadminReturn ar1 = asadminWithOutput("delete-deployment-group", cname);
    }
}
