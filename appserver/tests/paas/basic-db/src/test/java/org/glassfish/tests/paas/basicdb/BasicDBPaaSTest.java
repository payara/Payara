...
public class BasicDBPaaSTest {
    ...
    public void test() throws Exception {
        ...
        File archive = new File(System.getProperty("basedir") +
                "/target/basic_db_paas_sample.war"); // TODO :: use mvn apis to get the archive location.
        Assert.assertTrue(archive.exists());
        CommandRunner commandRunner = glassfish.getCommandRunner();
        CommandResult result = commandRunner.run("cloud-deploy",
                archive.getAbsolutePath());
        System.err.println("Deployed basic_db_paas_sample, command result = ["
                + result.getOutput() + "]");
        Assert.assertNull(result.getFailureCause());

        // 4. Access the app to make sure PaaS app is correctly provisioned.
        get("http://localhost:28080/basic_db_paas_sample/BasicDBPaaSServlet",
                "Request headers from the request:");

        // 5. Undeploy the PaaS application . TODO :: use cloud-undeploy??
        result = commandRunner.run("undeploy", "basic_db_paas_sample");
        System.err.println("Undeployed basic_db_paas_sample, command result = [" +
                result.getOutput() + "]");
