...
#asadmin create-template --files $SCRIPTS_DIR/OVM_EL5U6_X86_PVM_DERBY_DATABASE.tgz --indexes ServiceType=Database,VirtualizationType=OVM DERBY_DATABASE
#asadmin create-template-user --virtualization ovm --userid glassfish --groupid glassfish --template DERBY_DATABASE glassfish

asadmin create-template --files $SCRIPTS_DIR/OVM_JEOS_64_PVM_OTD_LARGE.tgz --properties vendor-name=otd --indexes ServiceType=LB,VirtualizationType=OVM otd
