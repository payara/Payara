...
@NotMicroCompatible
public class LookupIT extends AbstractClient {
    @Parameter
    ...
    public void ejbByBeanName() throws NamingException {
        ...
        RemoteService bean = getConnector().lookup(String.format("java:global%sRemoteServiceBean", uri.getPath()));
        assertThat(bean.createSimpleUser()).isInstanceOf(User.class);
    }

    @Test
    public void ejbByBeanAndInterfaceName() throws NamingException {
        ExposedService bean = getConnector().lookup(String.format("java:global%sHybridBean!%s", uri.getPath(),
                ExposedService.class.getName()));
        assertThat(bean.createSimpleUser()).isInstanceOf(User.class);
    }
