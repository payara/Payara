...
public class OpenTelemetryWithSpanIT extends AbstractSpanNameTest {
    ...
    public void testWithSpanAsyncException() throws Exception {
        ...
        // Span must be marked as an error with the error flag set
        assertEquals(Boolean.TRUE, span.getAttributes().get(AttributeKey.booleanKey("error")));
        // Other attributes (thread.id, thread.name, exception.type) may also be present
        // from Payara's ThreadStampingProcessor and WithSpanMethodInterceptor
        assertThat(span.getAttributes().asMap()).containsKey(AttributeKey.booleanKey("error"));
    }
