...
public class ContextServiceApplicationIT {
    private final static Logger logger = Logger.getLogger(ContextServiceApplicationIT.class.getName());
    ...
    public static Archive<?> createDeployment() {
        ...
        WebArchive webWar = ShrinkWrap.create(WebArchive.class, "test.war")
                .addClasses(ContextServiceApplication.class, ContextServiceConfigEJBRest.class);
    ...
    @Test
    @DisplayName("testing ContextService tag from Application EAR")
    @RunAsClient
    public void processContextServiceFromApplicationConfig() throws MalformedURLException {
        logger.log(Level.INFO, "Consuming service to test ContextService from application.xml config {0}",
                new Object[]{client});
    ...
    @Test
    @DisplayName("testing ContextService tag from ejb config")
    @RunAsClient
    public void processContextServiceFromEJBConfig() throws MalformedURLException {
        logger.log(Level.INFO, "Consuming service to test ContextService from application.xml config {0}",
                new Object[]{client});
        WebTarget target = this.client.target(new URL(this.base, "xml/ejb").toExternalForm());
        String message = target.request().accept(MediaType.TEXT_PLAIN).get(String.class);
        logger.log(Level.INFO, "Returned message {0}", new Object[]{message});
        //needs to be fixed because it is always returning 0
        assertTrue(message.contains("0"));
