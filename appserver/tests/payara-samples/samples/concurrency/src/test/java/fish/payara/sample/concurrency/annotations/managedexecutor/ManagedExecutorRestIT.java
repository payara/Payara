...
public class ManagedExecutorRestIT {
    private final static Logger logger = Logger.getLogger(ManagedExecutorRestIT.class.getName());
    ...
    @Test
    @DisplayName("testing ManagedExecutorDefinition annotation")
    @RunAsClient
    public void testManagedExecutorDefinition() throws MalformedURLException {
        logger.log(Level.INFO, "Consuming service to test ManagedExecutorDefinition annotation {0}",
                new Object[]{client});
        WebTarget target = this.client.target(new URL(this.base, "annotation/managedexecutor").toExternalForm());
    ...
    @Test
    @DisplayName("testing Multiple ManagedExecutorDefinition annotation")
    @RunAsClient
    public void testMultipleManagedExecutorDefinition() throws MalformedURLException {
        logger.log(Level.INFO, "Consuming service to test multiple ManagedExecutorDefinition annotation {0}",
                new Object[]{client});
        WebTarget target = this.client.target(new URL(this.base, "annotation/multiplemanagedexecutor").toExternalForm());
        String message = target.request().accept(MediaType.TEXT_PLAIN).get(String.class);
        logger.log(Level.INFO, "Returned message {0}", new Object[]{message});
        String[] data = message.split(":");
        if (data[1] != null) {
