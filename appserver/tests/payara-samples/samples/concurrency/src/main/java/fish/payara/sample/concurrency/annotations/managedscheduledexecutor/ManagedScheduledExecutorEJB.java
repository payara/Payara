...
@Stateless
public class ManagedScheduledExecutorEJB {

    public static final Logger logger = Logger.getLogger(ManagedScheduledExecutorEJB.class.getName());

    @Resource(lookup = "java:app/jakartaee/CustomManagedScheduledExecutorD")
    ManagedScheduledExecutorService customManagedScheduleExecutorD;

    public String processCustomManagedScheduled() throws InterruptedException, ExecutionException {
        logger.log(Level.INFO, String.format("Processing schedule executor: %s", customManagedScheduleExecutorD));
