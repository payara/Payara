...
public class DynamicRolesEJBTest {
    ...
    @Test
    @RunAsClient
    public void testAuthWithInlineMethodCall() {
        PersonControllerClient client = getPersonControllerClient(deploymentUrl, DEFAULT_USER, DEFAULT_PASSWORD);

        Person person = new Person(DEFAULT_USER, "Arjan", "Tijms", "user"); // convert method called to change accessType to uppercase
        Response response = client.createWithInlineMethodCall(person);
        assertThat(response, hasStatus(CREATED));
    }
