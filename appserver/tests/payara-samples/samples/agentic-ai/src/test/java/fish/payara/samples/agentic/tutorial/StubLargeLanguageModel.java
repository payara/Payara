...
/**
 * Test-only {@link LargeLanguageModel} returning deterministic HTML so the
 * integration test runs without a live LLM. It distinguishes a generation
 * prompt from a refinement prompt so the test can assert the refine loop works.
 */
@ApplicationScoped
public class StubLargeLanguageModel implements LargeLanguageModel {

    static final String GENERATED = "<section data-tutorial=\"generated\"><h3>Field guide</h3></section>";
    ...
    public <T> T query(String prompt, Class<T> resultType) {
        throw new UnsupportedOperationException("typed query not used by this sample");
    }
    ...
    public <T> T unwrap(Class<T> implClass) {
        throw new UnsupportedOperationException("unwrap not used by this sample");
    }
}
