 ...
 * holder.
 */
package fish.payara.monitoring.fang.handler;

import fish.payara.monitoring.fang.FangResponseToken;
import fish.payara.monitoring.fang.MBeanServerDelegate;
...
public abstract class ResourceHandler {
    ...
    public JSONObject getResource() {
        ...
        try {
            ...
            resourceResponse.put(FangResponseToken.getRequestKey(),
                    requestObject);

            Object valueObject = getValueObject();
            resourceResponse.put(FangResponseToken.getValueKey(), 
                    valueObject);
            ...
            if (errorThrown(status)) {
                JSONObject traceObject = (JSONObject) resourceResponse
                        .get(FangResponseToken.getValueKey());
               
                resourceResponse.put(FangResponseToken.getStacktraceKey(),
                        traceObject.get(FangResponseToken.getStacktraceKey()));
                resourceResponse.put(FangResponseToken.getErrorTypeKey(),
                        traceObject.get(FangResponseToken.getErrorTypeKey()));
                resourceResponse.put(FangResponseToken.getErrorKey(),
                        traceObject.get(FangResponseToken.getErrorKey()));
               
                resourceResponse.remove(FangResponseToken.getValueKey());
            } else {
                Long millis = System.currentTimeMillis();
                resourceResponse.put(FangResponseToken.getTimestampKey(),
                        millis);
            }

            int statusCode = status.getStatusCode();
            resourceResponse.put(FangResponseToken.getHttpStatusKey(),
                    statusCode);
    ...
    protected JSONObject getTraceObject(Exception exception) throws JSONException {
        ...
        traceObject.put(FangResponseToken.getStacktraceKey(), 
                stringWriter.toString());

        traceObject.put(FangResponseToken.getErrorTypeKey(), 
