...
public class FallbackValidator {
    ...
    public static void validateAnnotation(Fallback fallback, AnnotatedMethod<?> annotatedMethod) throws Exception {
        ...
        if (fallbackMethod != null && !fallbackMethod.isEmpty()) {
            ...
            if (fallbackClass.getDeclaredMethod(FALLBACK_HANDLER_METHOD_NAME).getReturnType() 
                    != annotatedMethod.getJavaMember().getReturnType()) {
                throw new FaultToleranceDefinitionException();
            }
        }
