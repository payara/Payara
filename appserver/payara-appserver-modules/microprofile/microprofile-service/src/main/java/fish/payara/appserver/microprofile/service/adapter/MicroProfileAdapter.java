...
/**
 * The adapter class for the MicroProfile application.
 * 
 * @author Andrew Pielage
 * @author Gaurav Gupta
 */
public abstract class MicroProfileAdapter extends HttpHandler implements Adapter {
    ...
    private static MicroProfileEndpointDecider endpointDecider;
    ...
    private final static Logger LOGGER = Logger.getLogger(MicroProfileAdapter.class.getName());
    private final static String RESOURCE_PACKAGE = "fish/payara/appserver/microprofile/service/adapter";
    ...
    protected void init() {
        try {
            endpointDecider = new MicroProfileEndpointDecider(serverConfig, getMicroProfileConfiguration());
        } catch (Exception ex) {
            LOGGER.log(Level.INFO, "MicroProfile Console cannot initialise", ex);
        }
        ...
        if (appExistsInConfig() && (domain.getSystemApplicationReferencedFrom(env.getInstanceName(), 
                getMicroProfileConfiguration().getApplicationName()) != null)) {
    ...
    public void service(Request request, Response response) throws Exception {
        ...
        try {
            if (!latch.await(100L, TimeUnit.SECONDS)) {
                LOGGER.log(Level.SEVERE, "Timed out processing a microprofile request");
                return;
            }
        } catch (InterruptedException ex) {
            LOGGER.log(Level.SEVERE, "Cannot process microprofile request");
            return;
        }
    ...
    private ResourceBundle getResourceBundle(Locale locale) {
        return ResourceBundle.getBundle(MicroProfileAdapter.class.getPackage().getName() + ".LocalStrings", locale);
    }
    ...
    private void handleResourceRequest(Request request, Response response) throws IOException {
        ...
        ClassLoader loader = MicroProfileAdapter.class.getClassLoader();
