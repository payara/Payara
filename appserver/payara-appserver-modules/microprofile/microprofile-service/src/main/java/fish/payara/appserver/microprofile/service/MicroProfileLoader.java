...
/**
 * Loader class for the MicroProfile application. Handles registering and actual
 * loading of the application.
 *
 * @author Andrew Pielage
 * @author Gaurav Gupta
 */
public class MicroProfileLoader extends Thread {

    private final Domain domain;
    private final ServerEnvironmentImpl serverEnv;
    private final String contextRoot;
    private String applicationName;
    private final MicroProfileAdapter microProfileAdapter;
    private final ServiceLocator serviceLocator;
    private final List<String> virtualServers;
    private final boolean dynamicStart;
    private static final Logger LOGGER = Logger.getLogger(MicroProfileLoader.class.getName());

    MicroProfileLoader(MicroProfileAdapter microProfileAdapter, ServiceLocator serviceLocator, Domain domain,
            ServerEnvironmentImpl serverEnv, String contextRoot, String applicationName, List<String> virtualServers) {
        this.microProfileAdapter = microProfileAdapter;
    ...
    public void run() {
        try {
            ...
            if (dynamicStart) {
                if (microProfileAdapter.appExistsInConfig(contextRoot)) {
                    // Check if we need to reconfigure system app to match the overriding config
                    if (!microProfileAdapter.isAppRegistered(contextRoot)) {
                        registerApplication();
                    } else {
                ...
                if (microProfileAdapter.appExistsInConfig()) {
                    // Check if the app is actually registered to this instance
                    if (!microProfileAdapter.isAppRegistered()) {
                        // We hit here if the app exists, but hasn't been registered to this instance yet
                        registerApplication();
                    } else if (!contextRoot.equals(microProfileAdapter.getSystemApplicationConfig().getContextRoot())) {
            ...
            LOGGER.log(Level.WARNING, "Problem while attempting to register or load the MicroProfile application!", ex);
        }
    }
    ...
    private void createAndRegisterApplication() throws Exception {
        LOGGER.log(Level.FINE, "Registering the MicroProfile Application...");
        ...
        ConfigCode code = proxies -> {
            ...
            application.setObjectType("system-all");
        ...
        LOGGER.log(Level.FINE, "MicroProfile Application Registered.");
    }
    ...
    private void registerApplication() throws Exception {
        ...
        ConfigCode code = (ConfigBeanProxy... proxies) -> {
            ...
            if (application == null) {
                throw new IllegalStateException("The MicroProfile application has no system app entry!");
            }
    ...
    private String getVirtualServerListAsString() {
        ...
        return String.join(",", virtualServers);
    }
    ...
    private void loadApplication() {
        ApplicationRegistry appRegistry = serviceLocator.getService(ApplicationRegistry.class);
        ApplicationInfo appInfo = appRegistry.get(applicationName);
        if (appInfo != null && appInfo.isLoaded()) {
            LOGGER.log(Level.FINE, "MicroProfile Application already loaded.");
            return;
        }

        Application config;
        if (dynamicStart) {
            config = microProfileAdapter.getSystemApplicationConfig(contextRoot);
        } else {
            config = microProfileAdapter.getSystemApplicationConfig();
        }
        ...
        // Load the MicroProfile Application
        ...
        Deployment lifecycle = serviceLocator.getService(Deployment.class);
        for (Deployment.ApplicationDeployment depl : serviceLocator
                .getService(ApplicationLoaderService.class)
                .processApplication(config, ref)) {
            lifecycle.initialize(depl.appInfo, depl.appInfo.getSniffers(), depl.context);
        }

        // Mark as registered
        microProfileAdapter.setAppRegistered(true);

        LOGGER.log(Level.FINE, "MicroProfile Application Loaded : {0}", applicationName);
    }
    ...
    private void checkAndResolveApplicationName(SystemApplications systemApplications) {
        ...
        if (applicationName == null || applicationName.equals("")) {
            LOGGER.log(Level.INFO, "No or incorrect application name detected for MicroProfile: reverting to default");
            applicationName = MicroProfileService.DEFAULT_MICROPROFILE_APP_NAME;
        }
    ...
    private void reconfigureSystemApplication() throws Exception {
        Application systemApplication = microProfileAdapter.getSystemApplicationConfig();

        LOGGER.log(Level.FINE, "Reconfiguring the MicroProfile Application...");

        // Reconfigure the system-application entry in the domain.xml
        ConfigCode code = proxies -> {
            Application systemApplication1 = (Application) proxies[0];
            systemApplication1.setContextRoot(contextRoot);
            return true;
        };

        ConfigSupport.apply(code, systemApplication);

        LOGGER.log(Level.FINE, "MicroProfile Application Reconfigured.");
    }
}
