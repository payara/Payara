...
/**
 * AsAdmin command to set dev console configuration
 *
 * @author Gaurav Gupta
 */
@Service(name = "set-dev-console-configuration")
@PerLookup
...
@RestEndpoints({
    @RestEndpoint(configBean = DevConsoleServiceConfiguration.class,
            opType = POST,
            path = "set-dev-console-configuration",
            description = "Sets the Dev Console Configuration")
})
public class SetDevConsoleConfigurationCommand extends SetSecureConsoleConfigurationCommand {

    private static final Logger LOGGER = Logger.getLogger(SetDevConsoleConfigurationCommand.class.getName());
    ...
    public void execute(AdminCommandContext context) {
        ...
        DevConsoleServiceConfiguration devConsoleConfiguration = targetConfig.getExtensionByType(DevConsoleServiceConfiguration.class);
        ...
        if (Boolean.TRUE.equals(securityEnabled)
                || Boolean.parseBoolean(devConsoleConfiguration.getSecurityEnabled())) {
        ...
        try {
            ConfigSupport.apply(configProxy -> {
                ...
                if (enabled != null) {
                    ...
                    if(dynamic != null && dynamic || Boolean.parseBoolean(devConsoleConfiguration.getDynamic())) {
                        devConsoleService.resetDevConsoleEnabledProperty();
                    }
                }
                if (endpoint != null) {
                ...
                return configProxy;
            }, devConsoleConfiguration);

            if(devConsoleConfiguration != null && !Boolean.valueOf(devConsoleConfiguration.getDynamic())) {
