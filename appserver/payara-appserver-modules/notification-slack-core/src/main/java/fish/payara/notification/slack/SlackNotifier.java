 ...
 * @author mertcaliskan
 */
@Service(name = "slack-notifier")
@RunLevel(StartupRunLevel.VAL)
public class SlackNotifier extends PayaraConfiguredNotifier<SlackNotifierConfiguration> {
    ...
    public void handleNotification(PayaraNotification event) {
        if (url == null) {
            LOGGER.fine("Slack notifier received notification, but no URL was available.");
            return;
        }
    ...
    public void bootstrap() {
        String formattedURL = MessageFormat.format(SLACK_RESOURCE,
                configuration.getToken1(),
                configuration.getToken2(),
                configuration.getToken3());
        ...
        mapper.registerModule(SlackSerializer.createModule());
    }
