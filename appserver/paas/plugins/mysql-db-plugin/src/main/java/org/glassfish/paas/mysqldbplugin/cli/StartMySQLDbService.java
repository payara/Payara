 ...
 * @author Shalini M
 */
@Service(name = "_start-mysql-db-service")
@Scoped(PerLookup.class)
@CommandLock(CommandLock.LockType.NONE)
public class StartMySQLDbService implements AdminCommand {
    ...
    private static Logger logger = Logger.getLogger(StartMySQLDbService.class.getName());
    ...
    public void execute(AdminCommandContext context) {
        ...
        if (serviceUtil.isValidService(serviceName, appName, DATABASE)) {
            synchronized (StartMySQLDbService.class) {
                ...
                try {
                    if(status == null || status.equalsIgnoreCase(Running.toString())) {
                        report.setMessage("MySQL-db-service [" + serviceName + "] already started");
                        report.setActionExitCode(ActionReport.ExitCode.FAILURE);
                        logger.log(Level.WARNING, "MySQL DB Service already running");
                        return;
                    }
                    ...
                    serviceUtil.updateState(serviceName, appName, Running.toString(), DATABASE);
                    report.setMessage("mysql-db-service [" + serviceName + "] started");
                    report.setActionExitCode(ActionReport.ExitCode.SUCCESS);
                    logger.log(Level.INFO, "Started MySQL DB Service Successfully");
                } catch (Exception ex) {
                    logger.log(Level.WARNING, "Exception  while starting mysql db service : " + ex);
                    report.setMessage("MySQL DB service [" + serviceName + "] start failed");
                    report.setActionExitCode(ActionReport.ExitCode.FAILURE);
                }
            }
        } else {
