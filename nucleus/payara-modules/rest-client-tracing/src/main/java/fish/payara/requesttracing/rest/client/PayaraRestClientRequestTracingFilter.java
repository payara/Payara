...
import static fish.payara.nucleus.requesttracing.domain.PropagationHeaders.PROPAGATED_PARENT_ID;
import static fish.payara.nucleus.requesttracing.domain.PropagationHeaders.PROPAGATED_RELATIONSHIP_TYPE;
import static fish.payara.nucleus.requesttracing.domain.PropagationHeaders.PROPAGATED_TRACE_ID;
...
public class PayaraRestClientRequestTracingFilter implements ClientRequestFilter {
    ...
    public void filter(ClientRequestContext requestContext) throws IOException {
        ...
        if (requestTracing!= null && requestTracing.isRequestTracingEnabled() && requestTracing.isTraceInProgress()) {
            ...
            if (!requestContext.getHeaders().containsKey(PROPAGATED_TRACE_ID)) {
                requestContext.getHeaders().add(PROPAGATED_TRACE_ID, requestTracing.getConversationID());
            }

            // Check that we aren't overwriting a header
            if (!requestContext.getHeaders().containsKey(PROPAGATED_PARENT_ID)) {
                requestContext.getHeaders().add(PROPAGATED_PARENT_ID, requestTracing.getStartingTraceID());
            }
            
            // Check that we aren't overwriting a relationship type
            if (!requestContext.getHeaders().containsKey(PROPAGATED_RELATIONSHIP_TYPE)) {
                if (requestContext.getMethod().equals("POST")) {
                    requestContext.getHeaders().add(PROPAGATED_RELATIONSHIP_TYPE, 
