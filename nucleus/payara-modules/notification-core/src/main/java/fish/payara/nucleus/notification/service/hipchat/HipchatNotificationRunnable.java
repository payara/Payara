 ...
 * @author mertcaliskan
 */
public class HipchatNotificationRunnable implements Runnable, Thread.UncaughtExceptionHandler {

    private static Logger logger = Logger.getLogger(HipchatNotificationRunnable.class.getCanonicalName());

    private static final String HTTP_METHOD_POST = "POST";
    private static final String ACCEPT_TYPE_TEXT_PLAIN = "text/plain";
    ...
    public void run() {
        while (queue.size() > 0) {
            String formattedURL = MessageFormat.format(RESOURCE, executionOptions.getRoomName(), executionOptions.getToken());
            ...
            try {
                ...
                connection.setRequestProperty("Content-Type", ACCEPT_TYPE_TEXT_PLAIN);
                connection.connect();
