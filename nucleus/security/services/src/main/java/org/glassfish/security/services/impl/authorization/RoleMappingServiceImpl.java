...
/**
 * <code>RoleMappingServiceImpl</code> implements <code>{@link RoleMappingService}</code> by
 * delegating role mapping decisions to configured <code>{@link RoleMappingProvider}</code>
 * instances.
 */
@Service
@Singleton
            ...
            if (config != null) {
                ...
                if (roleProviderConfig != null) {
                    ...
                    provider = AccessController
                        .doPrivileged(new PrivilegedLookup<>(serviceLocator, RoleMappingProvider.class, providerName));
