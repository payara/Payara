 ...
 * 
 * Delegate to the AuthRealm that is specified in the service configuration.
 */
@Service
@Singleton
public class AuthenticationServiceImpl implements AuthenticationService {
    
    private org.glassfish.security.services.config.AuthenticationService myConfig;
	private String realmName = "admin-realm";

    @Override
    public void initialize(org.glassfish.security.services.config.SecurityService serviceConfiguration) {
        myConfig = (org.glassfish.security.services.config.AuthenticationService) serviceConfiguration;

        realmName = myConfig.getAuthRealm();
    }

	@Override
	public Subject login(String username, char[] password, Subject subject)
			throws LoginException {
		// TODO - Initially delegate to existing API
	...
	public Subject impersonate(String user, String[] groups, Subject subject,
		...
		// TODO - Initially delegate to existing API
		LoginContextDriver.loginPrincipal(user, realmName);
		return SecurityContext.getCurrent().getSubject();
	}   
}
