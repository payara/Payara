...
public final class AMXValidator {
    ...
    private static final String NL = StringUtil.LS;
    ...
    public static final class ProblemList {
        ...
        private void add(final Throwable t) throws InstanceNotFoundException {
            add("", t);
        }
     ...
     */
    private static final Set<Class> EXTRA_ALLOWED_TYPES = SetUtil.newTypedSet( // any special-case exceptions go here
            );
    ...
    private static boolean isAcceptableRemoteType(final Class<?> c) {
        ...
        return c.isArray() && isAcceptableRemoteType(c.getComponentType());
    }

    /**
     * "best effort"
     * <p>
     * Attributes that cannot be sent to generic clients are not allowed. More than OpenTypes are allowed eg messy stuff like JSR 77 Stats and Statistics.
     */
    ...
    private void _validate(final AMXProxy proxy, final ProblemList problems) throws InstanceNotFoundException {
        ...
        try {
            ...
            if (!attrNames.equals(attributesMap.keySet())) {
                ...
                if (!keys.isEmpty()) {
                    throw new ValidationFailureException("Attributes Map contains attributes not found in the MBeanInfo: " + keys);
                }
            ...
            for (final AMXProxy child : childrenSet) {
                if (child.extra().singleton()) {
                    ...
                    if (!child.objectName().equals(proxy.child(childType).objectName())) {
                        throw new ValidationFailureException("Child type " + childType + " cannot be found via child(type)");
                    }
                }
            }
            ...
            for (final String type : childrenMaps.keySet()) {
                ...
                if (m.keySet().isEmpty()) {
                    throw new ValidationFailureException("Child type " + type + " has nothing in Map");
                }
            }
